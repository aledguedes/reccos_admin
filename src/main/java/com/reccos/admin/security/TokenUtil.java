...
//	private static final String HEADER_AUTH = "Authorization";
	...
	public static String encodeToken(User user) {
		...
		System.out.println("DEBUG encodeToken: " + user.getLogin());
	...
	public static Authentication decodeToken(HttpServletRequest request) {
		try {
			...
			System.out.println("DEBUG Entrei no decodeToken");
			
			String jwtToken = request.getHeader("Authorization");
			jwtToken = jwtToken.replace(TOKEN_HEADER, "");
			Jws<Claims> jswClaims = Jwts.parserBuilder().setSigningKey(SECRET_KEY.getBytes()).build().parseClaimsJws(jwtToken);
			...
			System.out.println("DEBUG - Erro ao decodificar o token");
		}
		return null;
